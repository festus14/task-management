...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.taskDocument.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.task-documents.update", [$taskDocument->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('document_type') ? 'has-error' : '' }}">
                ...
                <select id="document_type" name="document_type" class="form-control" required>
                    ...
                    @foreach(App\TaskDocument::DOCUMENT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('document_type', $taskDocument->document_type) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
