...
// plugin setup
function parseGermanDate(source) {
    var date = new Date(0);
    var year = parseInt(source.replace(/^\d{1,2}[^\d]\d{1,2}[^\d](\d{1,4})$/, "$1"));
    if (year < 30) year += 2000;
    if (year >= 30 && year < 100) year += 1900;
    date.setFullYear(year);
    date.setMonth(parseInt(source.replace(/^\d{1,2}[^\d](\d{1,2})[^\d]\d{1,4}$/, "$1")) - 1);
    date.setDate(parseInt(source.replace(/^(\d{1,2})[^\d]\d{1,2}[^\d]\d{1,4}$/, "$1")));
    return Date.parse(date.toGMTString());
}
